...
export function generateSecAppEmailHtml(options: SecAppEmailOptions): string {
  const {
    ...
    senderEmail,
    subject,
    appUrl = window?.location?.origin || 'https://secapp.eldorado.com.br'
  } = options;
...
<body style="margin: 0; padding: 0; background-color: #f1f5f9; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased; color: #1e293b;">
  ...
  <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f1f5f9; padding: 24px 12px;">
    <tr>
      <td align="center">
        <!-- Main Container Card -->
        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 16px rgba(15, 23, 42, 0.08); border: 1px solid #e2e8f0;">
          
          <!-- Top Accent Stripes (Eldorado Green & Gold) -->
          <tr>
            <td style="height: 6px; background: #059669; font-size: 0; line-height: 0;">&nbsp;</td>
          </tr>

          <!-- Header Section with SecApp Visual Brand -->
          <tr>
            <td style="padding: 24px 28px 18px 28px; background: linear-gradient(180deg, #ecfdf5 0%, #ffffff 100%); border-bottom: 1px solid #e2e8f0;">
          ...
          <tr>
            <td style="padding: 24px 28px;">
              ...
              <div style="background-color: #f0fdf4; border: 1px solid #a7f3d0; border-left: 4px solid #059669; border-radius: 10px; padding: 16px 18px; margin: 0 0 22px 0;">
 ...
 * When pasted into Outlook (desktop or web) or Gmail, it renders with full styling!
 */
export async function copyFormattedEmailToClipboard(options: SecAppEmailOptions): Promise<boolean> {
  const html = generateSecAppEmailHtml(options);
...
/**
 * Opens Outlook Web (Office 365 / Corporate or Live) compose window with pre-filled fields.
 */
export function openOutlookWeb(options: SecAppEmailOptions): void {
  ...
  // Deep-link to Microsoft 365 / Corporate Outlook Web (default in corporate environments like Eldorado)
  const officeUrl = `https://outlook.office.com/mail/deeplink/compose?to=${encodeURIComponent(to)}&subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  window.open(officeUrl, '_blank');
}

/**
 * Opens Gmail Web compose window with pre-filled fields.
 */
export function openGmailWeb(options: SecAppEmailOptions): void {
  const to = options.toEmail || '';
  const subject = options.subject || 'SecApp - Retorno de Avaliação';
  const body = generateSecAppEmailPlainText(options);
 ...
 */
export function openDefaultMailClient(options: SecAppEmailOptions): void {
  const to = options.toEmail || '';
  const subject = options.subject || 'SecApp - Retorno de Avaliação';
  const body = generateSecAppEmailPlainText(options);
