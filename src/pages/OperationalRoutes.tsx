...
const OperationalRoutes: React.FC = () => {
  ...
  const filteredHistSubmissions = useMemo(() => {
    ...
    return allSubmissionsMerged.filter(sub => {
      ...
      if (histSelectedDate) {
        // Compare targeted date (YYYY-MM-DD)
        const year = dateObj.getFullYear();
        const month = String(dateObj.getMonth() + 1).padStart(2, '0');
        const day = String(dateObj.getDate()).padStart(2, '0');
        const formattedItemDate = `${year}-${month}-${day}`;
        return formattedItemDate === histSelectedDate;
      }
    ...
    const startScanner = async () => {
      try {
        ...
        if (errStr.includes("notallowed") || errStr.includes("permission")) {
          setCameraError("Acesso à câmera negado. Por favor, permita o acesso nas configurações do navegador.");
        } else if (errStr.includes("notfound") || errStr.includes("device not found") || errStr.includes("requested device")) {
          setCameraError("Nenhuma câmera foi encontrada no dispositivo. Digite o código manualmente.");
        } else {
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider self-start sm:self-auto flex items-center gap-1.5">
                ...
                {filteredHistSubmissions.length} registradas
              </div>
            </div>

            {/* Professional Custom Date Filter Control - Sleek premium calendar button */}
            <div className="bg-slate-50 border border-slate-200/60 rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left">
              <div className="space-y-1">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1 select-none">
                  <Calendar className="w-3.5 h-3.5 text-[#0d6e4f]" /> Calendário Operacional:
                </span>
                <p className="text-[9px] text-slate-400 font-bold uppercase">
                  Exibindo por padrão Ontem e Hoje. Clique no botão ao lado para filtrar por uma data específica.
                </p>
              </div>

              <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
                {/* Premium Button Trigger with Hidden Native Date Picker */}
                <div className="relative overflow-hidden flex items-center justify-center bg-white border border-slate-200 hover:border-emerald-500 hover:bg-emerald-50/10 rounded-2xl px-4 py-2.5 transition-all shadow-xs group cursor-pointer w-full sm:w-auto min-w-[180px]">
                  ...
                  {/* Invisible Datepicker that perfectly overlaps the parent wrapper */}
                  <input
                    type="date"
                ...
                {histSelectedDate && (
                  <button
                    ...
                    className="p-2.5 bg-rose-50 hover:bg-rose-100 border border-rose-150 text-rose-600 rounded-2xl text-[10px] font-black uppercase tracking-wider transition-colors cursor-pointer"
                    title="Limpar filtro e voltar para Filtro Automático"
                  >
                    Limpar
                  </button>
                )}
            ...
            <div className="space-y-4">
              ...
              ) : filteredHistSubmissions.length === 0 ? (
                <div className="py-16 bg-white border border-dashed border-slate-200 rounded-[2rem] text-center text-slate-400 text-xs font-semibold flex flex-col items-center justify-center gap-2">
                  <History className="w-8 h-8 text-slate-300 animate-pulse" />
                  <span>Nenhum registro corresponde aos filtros definidos.</span>
                  <button
                    type="button"
                    onClick={() => {
                      setHistSelectedDate('');
                    }}
                    className="mt-1 text-[10px] font-black uppercase tracking-wider text-[#0d6e4f] hover:underline cursor-pointer"
                  >
                    Voltar para automático (Ontem & Hoje)
                  </button>
                </div>
              ) : (
                filteredHistSubmissions.map((sub, idx) => {
                  const dateObj = safeToDate(sub.createdAt);
                      ...
                      <div className="flex items-center gap-4 self-end md:self-auto">
                        ...
                        {(isManager || isAdmin || isMaster) && (
                          <button
                            ...
                            className="p-2 text-slate-350 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all cursor-pointer"
